import express from 'express';
import { Express } from 'express-serve-static-core';
import request from 'supertest';
import { Energi } from '../../../../src/chains/energi/energi';
import { Energiswap } from '../../../../src/connectors/energiswap/energiswap';
import { AmmRoutes } from '../../../../src/amm/amm.routes';
import { patch, unpatch } from '../../../services/patch';
import { gasCostInEthString } from '../../../../src/services/base';
let app: Express;
let energi: Energi;
let energiswap: Energiswap;

beforeAll(async () => {
  app = express();
  app.use(express.json());
  energi = Energi.getInstance('testnet');
  await energi.init();
  energiswap = Energiswap.getInstance('energi', 'testnet');
  await energiswap.init();
  app.use('/amm', AmmRoutes.router);
});

afterEach(() => {
  unpatch();
});

afterAll(async () => {
  await energi.close();
});

const address: string = '0x8704733aaAA1760ae31c2f3c962e817649260006';

const patchGetWallet = () => {
  patch(energi, 'getWallet', () => {
    return {
      address: '0x8704733aaAA1760ae31c2f3c962e817649260006',
    };
  });
};

const patchInit = () => {
  patch(energiswap, 'init', async () => {
    return;
  });
};

const patchStoredTokenList = () => {
  patch(energi, 'tokenList', () => {
    return [
      {
        chainId: 49797,
        name: 'WNRG',
        symbol: 'WNRG',
        address: '0x16c5074d9fc6afdbc021A8e44C8511d1A090F9AD',
        decimals: 18,
      },
      {
        chainId: 49797,
        name: 'DAI',
        symbol: 'DAI',
        address: '0x0B12E0D7397aA23549C3F546234817275FaEE889',
        decimals: 18,
      },
    ];
  });
};

const patchGetTokenBySymbol = () => {
  patch(energi, 'getTokenBySymbol', (symbol: string) => {
    if (symbol === 'WNRG') {
      return {
        chainId: 49797,
        name: 'WNRG',
        symbol: 'WNRG',
        address: '0x16c5074d9fc6afdbc021A8e44C8511d1A090F9AD',
        decimals: 18,
      };
    } else {
      return {
        chainId: 49797,
        name: 'DAI',
        symbol: 'DAI',
        address: '0x0B12E0D7397aA23549C3F546234817275FaEE889',
        decimals: 18,
      };
    }
  });
};

const patchGetTokenByAddress = () => {
  patch(energiswap, 'getTokenByAddress', () => {
    return {
      chainId: 49797,
      name: 'WNRG',
      symbol: 'WNRG',
      address: '0x16c5074d9fc6afdbc021A8e44C8511d1A090F9AD',
      decimals: 18,
    };
  });
};

const patchGasPrice = () => {
  patch(energi, 'gasPrice', () => 100);
};

const patchEstimateBuyTrade = () => {
  patch(energiswap, 'estimateBuyTrade', () => {
    return {
      expectedAmount: {
        toSignificant: () => 100,
      },
      trade: {
        executionPrice: {
          invert: jest.fn().mockReturnValue({
            toSignificant: () => 100,
            toFixed: () => '100',
          }),
        },
      },
    };
  });
};

const patchEstimateSellTrade = () => {
  patch(energiswap, 'estimateSellTrade', () => {
    return {
      expectedAmount: {
        toSignificant: () => 100,
      },
      trade: {
        executionPrice: {
          toSignificant: () => 100,
          toFixed: () => '100',
        },
      },
    };
  });
};

const patchGetNonce = () => {
  patch(energi.nonceManager, 'getNonce', () => 21);
};

const patchExecuteTrade = () => {
  patch(energiswap, 'executeTrade', () => {
    return { nonce: 21, hash: '000000000000000' };
  });
};

describe('POST /amm/price', () => {
  it('should return 200 for BUY', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patchGasPrice();
    patchEstimateBuyTrade();
    patchGetNonce();
    patchExecuteTrade();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        side: 'BUY',
      })
      .set('Accept', 'application/json')
      .expect(200)
      .then((res: any) => {
        expect(res.body.amount).toEqual('10000.000000000000000000');
        expect(res.body.rawAmount).toEqual('10000000000000000000000');
      });
  });

  it('should return 200 for SELL', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patchGasPrice();
    patchEstimateSellTrade();
    patchGetNonce();
    patchExecuteTrade();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        side: 'SELL',
      })
      .set('Accept', 'application/json')
      .expect(200)
      .then((res: any) => {
        expect(res.body.amount).toEqual('10000.000000000000000000');
        expect(res.body.rawAmount).toEqual('10000000000000000000000');
      });
  });

  it('should return 500 for unrecognized quote symbol', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DOGE',
        base: 'WNRG',
        amount: '10000',
        side: 'SELL',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 for unrecognized base symbol', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'SHIB',
        amount: '10000',
        side: 'SELL',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 for unrecognized base symbol with decimals in the amount and SELL', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'SHIB',
        amount: '10.000',
        side: 'SELL',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 for unrecognized base symbol with decimals in the amount and BUY', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'SHIB',
        amount: '10.000',
        side: 'BUY',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 when the priceSwapIn operation fails', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patch(energiswap, 'priceSwapIn', () => {
      return 'error';
    });

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DOGE',
        base: 'WNRG',
        amount: '10000',
        side: 'SELL',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 when the priceSwapOut operation fails', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patch(energiswap, 'priceSwapOut', () => {
      return 'error';
    });

    await request(app)
      .post(`/amm/price`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DOGE',
        base: 'WNRG',
        amount: '10000',
        side: 'BUY',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });
});

describe('POST /amm/trade', () => {
  const patchForBuy = () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patchGasPrice();
    patchEstimateBuyTrade();
    patchGetNonce();
    patchExecuteTrade();
  };
  it('should return 200 for BUY', async () => {
    patchForBuy();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
        nonce: 21,
      })
      .set('Accept', 'application/json')
      .expect(200)
      .then((res: any) => {
        expect(res.body.nonce).toEqual(21);
      });
  });

  it('should return 200 for BUY without nonce parameter', async () => {
    patchForBuy();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
      })
      .set('Accept', 'application/json')
      .expect(200);
  });

  it('should return 200 for BUY with maxFeePerGas and maxPriorityFeePerGas', async () => {
    patchForBuy();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
        nonce: 21,
        maxFeePerGas: '5000000000',
        maxPriorityFeePerGas: '5000000000',
      })
      .set('Accept', 'application/json')
      .expect(200);
  });

  const patchForSell = () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patchGasPrice();
    patchEstimateSellTrade();
    patchGetNonce();
    patchExecuteTrade();
  };
  it('should return 200 for SELL', async () => {
    patchForSell();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'SELL',
        nonce: 21,
      })
      .set('Accept', 'application/json')
      .expect(200)
      .then((res: any) => {
        expect(res.body.nonce).toEqual(21);
      });
  });

  it('should return 200 for SELL  with maxFeePerGas and maxPriorityFeePerGas', async () => {
    patchForSell();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'SELL',
        nonce: 21,
        maxFeePerGas: '5000000000',
        maxPriorityFeePerGas: '5000000000',
      })
      .set('Accept', 'application/json')
      .expect(200);
  });

  it('should return 200 for SELL with limitPrice', async () => {
    patchForSell();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'SELL',
        nonce: 21,
        limitPrice: '9',
      })
      .set('Accept', 'application/json')
      .expect(200);
  });

  it('should return 200 for BUY with limitPrice', async () => {
    patchForBuy();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
        nonce: 21,
        limitPrice: '999999999999999999999',
      })
      .set('Accept', 'application/json')
      .expect(200);
  });

  it('should return 500 for BUY with price smaller than limitPrice', async () => {
    patchForBuy();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
        nonce: 21,
        limitPrice: '9',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 for SELL with price higher than limitPrice', async () => {
    patchForSell();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'SELL',
        nonce: 21,
        limitPrice: '99999999999',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 404 when parameters are incorrect', async () => {
    patchInit();
    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: 10000,
        address: 'da8',
        side: 'comprar',
      })
      .set('Accept', 'application/json')
      .expect(404);
  });
  it('should return 500 when the priceSwapIn operation fails', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patch(energiswap, 'priceSwapIn', () => {
      return 'error';
    });

    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'SELL',
        nonce: 21,
        maxFeePerGas: '5000000000',
        maxPriorityFeePerGas: '5000000000',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });

  it('should return 500 when the priceSwapOut operation fails', async () => {
    patchGetWallet();
    patchInit();
    patchStoredTokenList();
    patchGetTokenBySymbol();
    patchGetTokenByAddress();
    patch(energiswap, 'priceSwapOut', () => {
      return 'error';
    });

    await request(app)
      .post(`/amm/trade`)
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
        quote: 'DAI',
        base: 'WNRG',
        amount: '10000',
        address,
        side: 'BUY',
        nonce: 21,
        maxFeePerGas: '5000000000',
        maxPriorityFeePerGas: '5000000000',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });
});

describe('POST /amm/estimateGas', () => {
  it('should return 200 for valid connector', async () => {
    patchInit();
    patchGasPrice();

    await request(app)
      .post('/amm/estimateGas')
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
      })
      .set('Accept', 'application/json')
      .expect(200)
      .then((res: any) => {
        expect(res.body.network).toEqual('testnet');
        expect(res.body.gasPrice).toEqual(100);
        expect(res.body.gasCost).toEqual(
          gasCostInEthString(100, energiswap.gasLimitEstimate)
        );
      });
  });

  it('should return 500 for invalid connector', async () => {
    patchInit();
    patchGasPrice();

    await request(app)
      .post('/amm/estimateGas')
      .send({
        chain: 'energi',
        network: 'testnet',
        connector: 'energiswap',
      })
      .set('Accept', 'application/json')
      .expect(500);
  });
});
